            ...
            <div class="mb-8">
              ...
              <div v-else class="space-y-4">
                <div 
                  ...
                  :key="element.tempId || element.id"
                  class="p-4 border border-gray-200 rounded-lg bg-gray-50"
                >
                  <div class="flex items-center justify-between mb-3">
                    <h4 class="font-medium text-gray-900">
                      Élément {{ index + 1 }}
                      <span v-if="element.id" class="text-sm text-gray-500">
                        (ID: {{ element.id }})
                      </span>
                    </h4>
                    <Button 
...
let nextElementId = 1000 // Commencer avec un ID élevé pour éviter les conflits

onMounted(() => {
  // Pré-remplir les données du modèle
  form.nom = props.modele.nom || ''
  form.description = props.modele.description || ''
  
  // Pré-remplir les éléments existants
  form.elements = props.elements.map(element => ({
    ...element,
    obligatoire: Boolean(element.obligatoire),
    valeur_min_attendue: element.valeur_min_attendue || null,
    valeur_max_attendue: element.valeur_max_attendue || null
  }))
})
...
const submit = () => {
  form.put(route('modeles-releves.update', {modeles_relefe: props.modele.id}))
}
</script> 